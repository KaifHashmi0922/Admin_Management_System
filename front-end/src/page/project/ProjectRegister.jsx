...
import { 
  FolderPlus, Mail, Phone, Tag, DollarSign, Calendar, Image, 
  FileText, ShieldCheck, CheckCircle2, ArrowLeft, Play, PauseCircle 
} from "lucide-react";
import useRegister from "../../api/hooks/Project/useProjectRegister";

export default function ProjectRegister() {
  const {
    formData, setFormData, handleChange, SubmitProject, toggleStatus
  } = useRegister();
          ...
          <div className="text-center space-y-3 pt-4">
            ...
            <div>
              <h1 className="text-2xl lg:text-3xl font-black text-slate-900 mb-1.5 bg-gradient-to-r from-slate-900 to-slate-800 bg-clip-text">
                Register Project <span className="text-emerald-600">➕</span>
              </h1>
              <p className="text-slate-600 text-sm lg:text-base max-w-md mx-auto">
                Fill in all details to create a new project
              </p>
            </div>
          </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-xs">
                      {formData.projectStatus ? "Project is active" : "Project is paused"}
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="absolute inset-0 flex items-center justify-between px-2.5 pointer-events-none z-10">
                    <span className={`text-[10px] font-bold text-white transition-opacity duration-300 ${
                      formData.projectStatus ? 'opacity-100' : 'opacity-0'
                    }`}>
                      Active
                    </span>
                    <span className={`text-[10px] font-bold text-white transition-opacity duration-300 ${
                      formData.projectStatus ? 'opacity-0' : 'opacity-100'
                    }`}>
                      Deactive
                    </span>
                  </div>
                  ...
                  <div 
                    className={`w-6 h-6 bg-white rounded-full shadow-lg transition-all duration-300 ease-in-out z-20 relative ${
                      formData.projectStatus ? 'translate-x-12' : 'translate-x-0'
                    }`}
                  />
                </button>
              </div>
